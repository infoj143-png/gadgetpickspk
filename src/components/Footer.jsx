...
import { ArrowUpRight, ShieldAlert, FileText, CheckCircle, Mail, MapPin, BookOpen } from 'lucide-react';
...
export default function Footer() {
  ...
  const guideLinks = [
    { label: 'Prayer Calculation Method', href: '/how-prayer-times-calculated' },
    { label: 'Ramadan Timing Guide 2026', href: '/ramadan-timing-guide' },
    { label: 'How to Find Qibla Direction', href: '/qibla-direction-guide' }
  ];
    ...
    <footer className="bg-slate-900 dark:bg-slate-950 text-slate-300 border-t-4 border-orange-500 pt-16 pb-8 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-lg font-black tracking-tight text-white">
                WaqtNama<span className="text-orange-500">PK</span>
              </span>
            </div>
            <p className="text-slate-400 text-sm leading-relaxed font-medium">
              Your trusted platform for accurate prayer timings, Islamic guides, and verified lifestyle recommendations in Pakistan.
            </p>
            <div className="flex items-center gap-2 text-xs bg-slate-800 dark:bg-slate-900 p-3 rounded-lg text-slate-400 border border-slate-700 dark:border-slate-800 font-semibold">
              ...
              <span>We provide accurate timetables and independent product reviews.</span>
            </div>
          </div>

          {/* Column 2: Quick Links & Timing Guides */}
          <div>
            <h4 className="text-white font-extrabold text-sm uppercase tracking-wider mb-5 pb-1 border-b border-slate-800 dark:border-slate-800 inline-block">
              Prayer & Timing Guides
            </h4>
            <ul className="space-y-3 text-sm font-semibold mb-6">
              {guideLinks.map((link) => (
                <li key={link.label}>
                  <Link
                    to={link.href}
                  ...
                  >
                    <BookOpen size={14} className="text-orange-500 transition-colors" />
                    {link.label}
                  </Link>
                </li>
              ))}
            ...
            <h4 className="text-white font-extrabold text-xs uppercase tracking-wider mb-3 text-slate-400">
              Product Comparisons
            </h4>
            <ul className="space-y-2 text-xs font-semibold">
              {quickLinks.slice(0, 3).map((link) => (
                <li key={link.label}>
                  <Link
                    to={link.href}
                    className="hover:text-orange-500 dark:hover:text-orange-400 transition-colors flex items-center gap-1 group text-slate-400"
                  >
                    <ArrowUpRight size={12} className="text-slate-600 dark:text-slate-500 group-hover:text-orange-500 dark:group-hover:text-orange-400 transition-colors" />
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm text-slate-400 font-semibold">
              ...
              <li className="flex gap-2">
                ...
                <Link to="/contact" className="hover:text-orange-500 dark:hover:text-orange-400 transition-colors">
        ...
        <div className="border-t border-slate-800 dark:border-slate-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-slate-500 font-semibold">
          <div>
            &copy; {currentYear} <span className="text-slate-400">WaqtNama</span>. All Rights Reserved. Made for Pakistan with ❤️.
          </div>
